...
if ($rvid) {
    ...
    if ($op == 'save') {
        ...
        $num    = ($data['status'] != _RVSTAT_REFUSED) ? (isset($vals[$nlab]) ? $vals[$nlab] : 1) : 0;
        ...
        $nnum = ($status != _RVSTAT_REFUSED) ? (isset($vals[$nlab]) ? $vals[$nlab] : 1) : 0;
...
switch ($op) {
    ...
    case 'one':
        ...
        $GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_confirm.html';
        ...
        if ($data['uid']) {
            $items[] = array('label' => _MD_UNAME, 'value' => XoopsUserUtility::getUnameFromId($data['uid']));
        }
        if ($rvdata['operator']) {
            $items[] = array(
    ...
    default:
        $GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_receipt.html';
        ...
        $xoopsTpl->assign(array(
                              ...
                              'labels'      => array_merge(array($xoopsModuleConfig['member_only'] ? _MD_UNAME : _MD_EMAIL), array_slice($item, 0, $max)),
                              'reserv_msg'  => $mailmsg,
                              'operations'  => array(
