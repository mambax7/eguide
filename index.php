...
if (empty($prev)) {
    ...
    $cond .= " AND ((expire>=edate AND expire>$now) || (expire<edate AND (IF(exdate,exdate,edate)+expire)>$now))";
...
$GLOBALS['xoopsOption']['template_main'] = EGPREFIX . '_index.html';
...
if (count($catlist) > 1) {
    ...
    $xoopsTpl->assign('categories', array(
        'options' => $catlist,
        'action'  => 'index.php',
