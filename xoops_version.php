...
if (!file_exists("$modulePath/$myicon")) {
...
if ($configs) {
    if (!empty($configs['user_notify'])) {
        $modversion['sub'][] = array('name' => _MI_EGUIDE_REG, 'url' => 'reserv.php?op=register');
    }
}
// login users
if (is_object($xoopsUser)) {
    $modversion['sub'][] = array('name' => _MI_EGUIDE_MYLIST, 'url' => 'mylist.php');
    // poster administration
    if (!empty($configs) && in_array($configs['group'], $xoopsUser->getGroups())) {
        $modversion['sub'][] = array('name' => _MI_EGUIDE_SUBMIT, 'url' => 'admin.php');
        $modversion['sub'][] = array('name' => _MI_EGUIDE_COLLECT, 'url' => 'collect.php');
    }
}
